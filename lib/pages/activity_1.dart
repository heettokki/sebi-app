...
class Activity1Page extends StatefulWidget {
  ...
  const Activity1Page({super.key, required this.phrases});
...
class _Activity1PageState extends State<Activity1Page> {
  ...
  // Add a list of choices for each question
  final List<List<String>> choicesList = [
    ["Kamusta", "Maayong buntag", "Maayong hapon", "Ba-bay"],
    ["Maayong buntag", "Kamusta", "Maayong gabii", "Ba-bay"],
    ["Maayong hapon", "Maayong buntag", "Kamusta ka?", "Maayo ra ko"],
    ["Maayong gabii", "Maayong hapon", "Kamusta", "Ba-bay"],
    ["Ba-bay", "Kamusta ka?", "Maayo ra ko", "Maayong buntag"],
    ["Kamusta ka?", "Maayo ra ko", "Maayong hapon", "Ba-bay"],
    ["Maayo ra ko", "Kamusta ka?", "Maayong buntag", "Maayong gabii"],
    [
      "Unsa imong ngalan?",
      "Akong ngalan kay (Name)",
      "Nalipay ko nga nagkaila ta",
      "Maayo ra ko",
    ],
    [
      "Akong ngalan kay (Name)",
  ...
  void _setQuestionType() {
    // Alternate between fill-in and multiple choice for demo
    setState(() {
  ...
  void _nextQuestion() {
    if (currentIndex < widget.phrases.length - 1) {
      ...
      setState(() {
        showResult = true;
        showFailPrompt = score < (widget.phrases.length * 7.5); // 75% threshold
      });
    }
  }

  List<String> choices() {
    // Use the fixed choices for each question
    if (currentIndex < choicesList.length) {
      return choicesList[currentIndex];
    }
    // Fallback to all Cebuano phrases if out of bounds
    final correct = widget.phrases[currentIndex]['ceb']!;
            ...
            Padding(
              ...
              child: Column(
                children: List.generate(choices().length, (i) {
                  ...
                  // Show correct/wrong prompt and highlight correct answer if wrong
                  if (showCorrect &&
                      selectedChoice == i &&
                      choice == phrase['ceb']) {
